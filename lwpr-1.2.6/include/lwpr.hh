...
class LWPR_Object {
   ...
   */
   ...
   void computeJacobianNumerical(const doubleVec& x, Eigen::MatrixXd& jacobian)
   {
	   doubleVec fOriginal = this->predict(x);
	   doubleVec fPerturbed;
	   doubleVec xPerturbed, xTemp;
	   ...
	   {
			//perturb input one-by-one   
			xTemp = x;
			xTemp[i] += h;
		
			fPerturbed = this->predict(xTemp);

			doubleVec fDiff = fPerturbed - fOriginal;

			jacobian.col(i) = invh * Eigen::Map<Eigen::VectorXd> (fDiff.data(), fDiff.size());
