...
CCTRDoc::CCTRDoc()
{
	...
	::std::string pathToForwardModel("../models/model_ct_2015_10_27_9_51_31.bin");
	::std::string pathToInverseModel("C:/03. OnlineCalibration/OnlineCalibLWPR/models/model_ct_inverse_2015_10_16_18_26_34.bin");
	
	m_kinLWPR = new LWPRKinematics(pathToForwardModel, pathToInverseModel);
	double forgettingFactor[2] = {0.99, 0.99};
...
void CCTRDoc::OnBnClickedRegst()
{
	...
	char fName[100] = "C:\\03. OnlineCalibration\\OnlineCalib\\RegPointSet.txt";
...
/*			if (mySelf->m_adapt_LWPR)
				mySelf->m_kinLWPR->AdaptInverseModel(localStat.sensedTipPosDir, localStat.currJang);
*/

			if (mySelf->m_adapt_LWPR)
				mySelf->m_kinLWPR->AdaptForwardModel(localStat.currTipPosDir, localStat.currJang);
...
void CCTRDoc::InvKinJangToMtr(const double* jA, const double* currCnt, double* tgtJang, double* tgtCnt, bool& isInLimFlag)
{
	...
	if(L31 > L31_MAX)		{	L31 = L31_MAX;	isInLimFlag = false;					}
	if(L31 < L31_MIN)		{	L31 = L31_MIN;	isInLimFlag = false;					}
	...
	if(L1 > 100.0)						{	L1 = 100.0;						isInLimFlag = false;	}
	if(L1 < (-100.0 + L31_MAX - L31))	{	L1 = (-100.0 + L31_MAX - L31);	isInLimFlag = false;	}
...
void CCTRDoc::SolveInverseKin(CTR_status& stat)
{
	...
	//this solves the inverse kinematics using the inverse model of LWPR

	double lsq = 0;
	int exitC = 0;

	//m_kinLWPR->TipFwdKin(stat.currJang, stat.posOrLWPR);
	//m_kinLWPR->InverseKinematicsRootFinding(stat.tgtTipPosDir, stat.initJang, stat.jAngLWPR, &lsq, exitC);
	//::std::cout << stat.jAngLWPR[0] << ::std::endl;
